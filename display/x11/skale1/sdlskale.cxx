...
#define ZEIGERANSCHLAG_LINKS 130
#define ZEIGERANSCHLAG_RECHTS 890

#define SKALE_HOEHE 337
...
int main(void) {
   ...
   SDL_Rect ZeigerRect;
   ZeigerRect.x = 500; ZeigerRect.y = 0;
   ...
   while (1) {
	...
	if (labs(ZeigerRect.x - ((SENDERABSTAND*i) + ZEIGERANSCHLAG_LINKS)) > delta)
    	...
    	if (ZeigerRect.x > (SENDERABSTAND*i) + ZEIGERANSCHLAG_LINKS ) {
        	...
        	//ZeigerRect.x--;
                ZeigerRect.x = ZeigerRect.x - delta;
    	...
    	SDL_Rect dstrect = { 290, 315, texW, texH };
