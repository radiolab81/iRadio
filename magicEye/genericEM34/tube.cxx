...
#define ROEHREN_HOEHE 320
#define ROEHREN_BREITE ROEHREN_HOEHE
...
// oeffnet und schliesst das magische Auge [0..100%]
void SDL_DrawShadow(SDL_Renderer* renderer, int iPercent) {
   if ((iPercent >=0) | (iPercent<=100)) {
      SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
      int x,y,radius = ROEHREN_BREITE;
      for (float angle = 0.0; angle<(iPercent/100.0)*3.1415; angle=angle+0.001)
    	 {
	    x = radius * cos(angle);
	    y = radius * sin(angle);
            SDL_RenderDrawLine(renderer,ROEHREN_BREITE/2,ROEHREN_HOEHE/2, (ROEHREN_BREITE/2)+x, (ROEHREN_HOEHE/2)+y);
            SDL_RenderDrawLine(renderer,ROEHREN_BREITE/2,ROEHREN_HOEHE/2, (ROEHREN_BREITE/2)-x, (ROEHREN_HOEHE/2)-y);
         }
   } // if ((iPercent >=0) | (iPercent<=100)) {
} //void SDL_DrawShadow(SDL_Renderer* renderer, int iPercent) {
...
int main(void) {
    ...
    textur_tube = IMG_LoadTexture(renderer, "/home/pi/iRadio/magicEye/genericEM34/tube.png");
    textur_tubemitte = IMG_LoadTexture(renderer, "/home/pi/iRadio/magicEye/genericEM34/tube2.png");
   ...
   DestR.x = (int) (ROEHREN_BREITE/2-(DestR.w/2))+5; DestR.y = (int) (ROEHREN_HOEHE/2-(DestR.h/2))+2;
   ...
   while (1) {
        ...
        SDL_DrawShadow(renderer, 100-atoi(buf)); 
